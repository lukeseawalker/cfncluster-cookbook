...
# Install only on Centos/RHEL 7.6 and 7.5
if node['platform_version'] == 'rhel' && (5..6).cover?(node['platform_version'].split('.')[1].to_i)
  ...
  kernel_module 'lnet'
elsif node['platform_version'] == 'rhel' && node['platform_version'].split('.')[1].to_i == 7
  ...
  kernel_module 'lnet'
elsif node['platform_version'] == 'rhel'
  Chef::Log.warn("Unsupported version of CentOS/RHEL, #{node['platform_version']}, supported versions are 7.5, 7.6 and 7.7")
